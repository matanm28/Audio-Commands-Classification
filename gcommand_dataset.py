...
def make_dataset_test(dir):
    ...
    for root, _, fnames in sorted(os.walk(dir)):
        fnames = [(int(name[0:-4]), name) for name in fnames[1:]]
        for _, fname in sorted(fnames, key=lambda x: x[0]):
            if is_audio_file(fname):
                path = os.path.join(root, fname)
                item = (path, None)
                spects.append(item)
...
class GCommandLoader(data.Dataset):
    ...
    def __init__(self, root, transform=None, target_transform=None, window_size=.02,
        ...
        spects = make_dataset(root, class_to_idx) if not is_test else make_dataset_test(root)
